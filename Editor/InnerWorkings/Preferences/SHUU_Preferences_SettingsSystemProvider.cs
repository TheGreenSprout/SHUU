#if UNITY_EDITOR
using System.IO;
using UnityEditor;
using UnityEngine;
using UnityEngine.UIElements;

using SHUU.Utils.SettingsSystem;

using static SETB.EditorGUI_Base;

namespace SHUU._Editor.InnerWorkings
{
    public class SettingsSystem_ProjectSettingsProvider : SettingsProvider
    {
        #region Variables
        private SerializedObject _serializedObject;

        private SHUU_Preferences_SettingsSystem _preferences => SHUU_Preferences_SettingsSystem.instance;



        [SettingsProvider]
        public static SettingsProvider CreateProvider() => new SettingsSystem_ProjectSettingsProvider("Project/SHUU/Settings System", SettingsScope.Project);
        #endregion




        #region Main
        public SettingsSystem_ProjectSettingsProvider(string path, SettingsScope scope) : base(path, scope) { }


        public override void OnActivate(string searchContext, VisualElement rootElement) => _serializedObject = new SerializedObject(_preferences);


        public override void OnGUI(string searchContext)
        {
            if (_preferences == null)
            {
                EditorGUILayout.HelpBox("Preferences asset missing.", MessageType.Error);
                return;
            }

            _serializedObject.Update();

            float previousLabelWidth = EditorGUIUtility.labelWidth;
            EditorGUIUtility.labelWidth = 400f;

            DrawCategory("Settings System",
                "defaultAsset");

            EditorGUIUtility.labelWidth = previousLabelWidth;

            _serializedObject.ApplyModifiedProperties();

            if (GUI.changed) EditorUtility.SetDirty(_preferences);
        }
        #endregion
        


        #region Logic
        private void DrawCategory(string title, params string[] properties)
        {
            Space(10);

            Vertical(() =>
            {
                DrawLabel(title, EditorStyles.boldLabel);

                foreach (string propertyName in properties)
                {
                    SerializedProperty property = _serializedObject.FindProperty(propertyName);
    
                    if (property != null) DrawInputProperty(null, property);
                }
            }, "box");
        }
        #endregion
    }
}
#endif
