...
public class TypewriterText : MonoBehaviour
{
    ...
    public DialogueSounds currentTextSounds = null;

    public void SetTextSounds(DialogueSounds sounds) => currentTextSounds = sounds;
    ...
    private void Update()
    {
        if (Input.GetMouseButtonDown(1)) // Right click to skip
        {
            if (!_readyForNewText)
            {
                Skip();
        ...
        if (Input.GetMouseButtonDown(0)) // Left click to continue to next line
        {
            if (_readyForNewText)
            {
                _textBox.text = "";
