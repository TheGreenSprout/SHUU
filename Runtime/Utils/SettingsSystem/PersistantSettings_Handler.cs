...
namespace SHUU.Utils.SettingsSystem
{
    public class PersistantSettings_Handler : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            RunUpdates(item => item.updateOnFirstStart);
        }
        ...
        private void OnSceneLoaded(Scene scene, LoadSceneMode mode) => RunUpdates(item => item.updateOnSceneChange);
        ...
        private void RunUpdates(Func<SettingsUpdate_Item, bool> predicate)
        {
            if (settingsUpdate == null) return;
            ...
            foreach (var update in settingsUpdate)
            {
                ...
                foreach (var map in update.maps)
                {
                    ...
                    foreach (var item in map.items)
                    {
                        if (item.updateEvent == null || !predicate(item)) continue;

                        item.updateEvent.Invoke(update.settingsData, map.mapName, item.field);
                    }
                }
            }
        }
    ...
    public class SettingsUpdate_Item
    {
        ...
        public bool updateOnFirstStart = true;
