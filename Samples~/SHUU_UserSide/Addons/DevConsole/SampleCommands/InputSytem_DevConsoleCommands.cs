...
public class InputSytem_DevConsoleCommands : MonoBehaviour
{
    ...
    private static CommandReturn RetrieveAction(string _actionPath, out InputAction action)
    {
        ...
        if (action == null) return CommandReturn.Red($"Action '{_actionPath}' doesn't exist.");
    ...
    public static CommandReturn Rebind(string _actionPath, int _bindingIndex, string _newControlPath)
    {
        ...
        if (_bindingIndex < 0 || _bindingIndex >= action.bindings.Count) return CommandReturn.Red($"Binding index {_bindingIndex} out of range (action has {action.bindings.Count} binding(s)).");


        action.ApplyBindingOverride(_bindingIndex, _newControlPath);

        return CommandReturn.Green($"'{_actionPath}' binding {_bindingIndex} overridden to '{_newControlPath}'.");
    }
    ...
    public static CommandReturn ResetBind(OptionalParameter<string> _actionPath, OptionalParameter<int> _bindingIndex)
    {
        if (_actionPath.TryGetValue(out string actionPath))
        {
            ...
            {
                if (bindingIndex < 0 || bindingIndex >= action.bindings.Count) return CommandReturn.Red($"Binding index {bindingIndex} out of range (action has {action.bindings.Count} binding(s)).");

                action.RemoveBindingOverride(bindingIndex);

                return CommandReturn.Green($"'{actionPath}' binding {bindingIndex} reset to default.");
            }


            action.RemoveAllBindingOverrides();

            return CommandReturn.Green($"All bindings on '{actionPath}' reset to default.");
        }


        SHUU_Input.ResetToDefaults();

        return CommandReturn.Green("All bindings reset to default.");
    }
    #endregion
    ...
    public static CommandReturn AllInputMaps()
    {
        ...
        if (names.Length == 0) return CommandReturn.Red("No action maps found.");
    ...
    public static CommandReturn InputMap(string _map)
    {
        ...
        if (map == null) return CommandReturn.Red($"Map '{_map}' doesn't exist or isn't assigned.");

        if (map.actions.Count == 0) return CommandReturn.Red("No actions registered on this map.");
    ...
    public static CommandReturn InputAction(string _actionPath)
    {
        ...
        if (action.bindings.Count == 0) return CommandReturn.Red("No bindings registered on this action.");
    ...
    public static CommandReturn BindCommandDirect(string _controlPath, params string[] commandData)
    {
        ...
        if (control == null) return CommandReturn.Red($"Invalid control path '{_controlPath}'");

        BoundCommands.BindDirectCommand(_controlPath, commandData);

        return CommandReturn.Green($"Command bound to '{_controlPath}' successfully.");
    }
    ...
    public static CommandReturn BindCommand(string _actionPath, params string[] commandData)
    {
        ...
        return CommandReturn.Green($"Command bound to '{_actionPath}' successfully.");
    }
